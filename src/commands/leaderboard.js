...
async function getWeaponLeaderboard(type) {
    return fetch(`${functions.apiURLBase}user/weapon-ranking/${type}`)
        .then(res => {
            return res.json()
        })
}
...
async function getSkillXPLeaderboard() {
    return fetch(`${functions.apiURLBase}user/ranking`)
        .then(res => {
            return res.json()
        })
}
...
async function getWeaponRanking() {
    return fetch(`${functions.apiURLBase}user/weapon-ranking`)
        .then(res => {
            return res.json()
        })
}
        ...
        if (!choices.includes(type)) {
            ...
            if (weapon['success'] == true) {
                ...
                if (leaderboard == null) {
                    ...
                    for (i = 0; i < 10; i++) {
                        formatedLeaderboard += `**#${i + 1}** ${leaderboard[i]['name'].replace(/_/g, '\\_')}\n᲼᲼\`${nf.format(leaderboard[i]['kills'])} kills\`\n`
                    }
                }
                formatedLeaderboard = formatedLeaderboard.replace(/#1/, '🥇')
                formatedLeaderboard = formatedLeaderboard.replace(/#2/, '🥈')
